...
module.exports = {
    approval_shows_id: async (req, res) => {
        try {
            const { id } = req.params;
            const sqlRequests = `
                SELECT 
...
WHERE r1.request_id = @request_id; 
            ...
            res.status(200).json(resultRepuests);
        } catch (error) {
    ...
    approval_data_details: async (req, res) => {
        // ยังไม่ได้นำไปใช้ในส่วนไหน
        try {
            const sql = `
    SELECT 
    r1.request_id,
...
WHERE r1.request_id = @request_id; 
            ...
            const result = await executeSelectQuery(sql, value);
