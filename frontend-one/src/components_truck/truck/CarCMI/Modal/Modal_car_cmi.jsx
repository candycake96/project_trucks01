...
const Modal_car_cmi = ({ isOpen, onClose }) => {
  ...
  const fetchTaxend = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/cmi_managment_show`, {
  ...
  const filteredData = (isDataTaxend || []).filter((row) => {
    const reg = (row?.reg_number || "").toString().toLowerCase();
    const carType = (row?.car_type_name || "").toString().toLowerCase();
    ...
    >
      ...
      >
        <div className="d-flex align-items-center">
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 18 }}>ข้อมูลรถ - พรบ. (พระราชบัญญัติคุ้มครองผู้ประสบภัยจากรถ)</div>
            <div style={{ fontSize: 12, opacity: 0.9 }}>สรุปสถานะ พรบ. ของยานพาหนะ</div>
          </div>
        </div>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((row, index) => (
                  <tr key={index}>
                    ...
                    <td className="fw-semibold">{row?.reg_number || "-"}</td>
                    <td>{row?.car_type_name || "-"}</td>
                    <td>{formatDate(row?.act_date_end)}</td>
                    <td className="text-center">
                      {row?.tax_status === "หมดอายุ" ? (
                        <span className="badge bg-danger">{row?.act_status}</span>
                      ) : row?.act_status === "ใกล้หมดอายุ" ? (
                        <span className="badge bg-warning text-dark">{row?.act_status}</span>
                      ) : row?.act_status === "ยังไม่มีข้อมูล" ? (
                        <span className="badge bg-secondary">{row?.act_status}</span>
                      ) : (
                        <span className="badge bg-success">{row?.act_status}</span>
                      )}
                    </td>
                    <td className="text-center">
                      <Link to="/truck/VehicleShowDataDetails" state={row} className="btn btn-sm btn-outline-primary">
